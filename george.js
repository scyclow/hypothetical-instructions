function georgePath(x, y, d, f="none") {
  console.log(x, y)
  ...
  path.setAttribute('style', `transform: translate(${x}px, ${y}px) scale(1.5)`)
...
const hearts = (x, y) => (drawn.hearts = true, svg.drawPath(x, y, 'M10 16.5L20 9L17 3.5L12.6738 9.71888M12.6738 9.71888L10.3233 13.0977M12.6738 9.71888L11 10.5L6.5 3.5L3 8L9 15L10.3233 13.0977M12.6738 9.71888L18.5 7L11 14L10.3233 13.0977M10.3233 13.0977L5 6M52 16.5L62 9L59 3.5L54.6738 9.71888M54.6738 9.71888L52.3233 13.0977M54.6738 9.71888L53 10.5L48.5 3.5L45 8L51 15L52.3233 13.0977M54.6738 9.71888L60.5 7L53 14L52.3233 13.0977M52.3233 13.0977L47 6M11.5297 19C13.7548 13.8277 21 16.0492 21 7.65301C21 -0.743188 14.2441 1.02626 11.5297 7.65301C8.8691 0.469987 2.91961 0.516622 2.05884 7.65303C1.19807 14.7894 10.0421 15.5634 11.5297 19ZM53.5297 19C55.7548 13.8277 63 16.0492 63 7.65301C63 -0.743188 56.2441 1.02626 53.5297 7.65301C50.8691 0.469987 44.9196 0.516622 44.0588 7.65303C43.1981 14.7894 52.0421 15.5634 53.5297 19Z') )
...
const devilHorns = (x, y) => (drawn.devilHorns = true, svg.drawPath(x, y, 'M35.4914 75C7.14021 72.8917 -9.90881 24.6777 11.4938 2C0.523006 10.6718 18.5087 52.354 45.4922 53.5M112.992 58.5C127.411 57.4814 141.448 35.5505 138.492 18.5C148.962 31.8447 139.501 71.284 123.492 75') )
const laserEyes = (x, y) => (drawn.laserEyes = true, svg.drawPath(x, y, 'M52 225.5L5.5 9.5M5.5 9.5L82.5 249M5.5 9.5L63 225.5M132 231L51 9.5M51 9.5L167 249M51 9.5L144 225.5M4 3.5C4 3.5 4.82145 7.38905 0.5 7.5C4.47272 6.96133 5.63473 8.84932 2 13.5C4.98321 9.6911 6.55394 9.41585 9 15.5C6.44207 10.1092 6.64295 8.14132 13 8.5C7.89073 8.13069 6.96761 6.93684 10.5 2C7.91268 6.15767 4 3.5 4 3.5ZM49 4.5C48.3386 6.7922 46.4553 7.45798 44.5 7C47.2433 8.06978 47.115 9.12035 45 11.5C47.4353 9.91167 49.2133 11.0369 50 13.5C48.3994 10.1406 50.299 8.59907 53.5 7C50.3831 9.25969 48.7515 8.82817 49 4.5Z') )
...
const bowtie = (x, y) => (drawn.bowtie = true, svg.drawPath(x, y, 'M34.25 16.25L66.5 3V29.5L34.25 16.25ZM34.25 16.25L2 29.5V3L34.25 16.25ZM39 16C39 18.2091 37.5637 19.2408 34.25 19.2408C30.9363 19.2408 29 18.2091 29 16C29 13.7909 30.9363 13.2392 34.25 13.2392C37.5637 13.2392 39 13.7909 39 16Z') )
...
const clownNose = (x, y) => {
  ...
  times(9, i => {
    svg.drawCircle(x, y, i*3-2, {stroke: 'red'})
  })
}
...
function drawFace() {
  ...
  if (drawHair) chance(
    [0.7, () => hair(720, 186, '#ff0')],
    [0.1, () => mohawk(730, 63, '#f00')],
    [0.1, () => devilHorns(778, 158)],
    [0.1, () => headband(756, 235)],
  )()

  if (drawTorso) blouse(796, 380, '#0f0')


  if (drawEyes) chance(
    [2, () => glasses(824, 300, 'red')],
    [2, () => eyePatch(827, 280)],
    [2, () => xEyes(866, 301)],
    [2, () => $Eyes(873, 304)],
    [1, () => eyeLashes(855, 296)],
  ...
  if (drawFaceTattoo && !drawn.headband) {
    svg.drawRect(878, 240, 62, 33)
    svg.text('0', 878+5, 240+2)

    // $, heart, star, 666, dick, cgk, rosette, IOU
  }
}